...
public:
	...
	int   GetMatchCount(char* pWord);           // Find the number of matches in the dictionary, for a partial word
	void  Reset()                               // Reset dictionary
	{
		mDictionaryCount = 0;
	}

	int   isWordContained(char* pWord)
	{
		for(int i = 0; i < mDictionaryCount; i++)
			if(strcmp(mDictionary[i],pWord) == 0)
				return 1;
