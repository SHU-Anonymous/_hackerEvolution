...
hedRendererFont fontSmall,fontLarge;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// * Draw a 2D antialiased line
void render_DrawLine(int x1, int y1, int x2, int y2, int r, int g, int b, int transparent)
{
	SDL_SetRenderDrawColor(sdlRenderer,r,g,b,transparent);
...
void render_DrawRectangle(int x, int y, int width, int height, int r, int g, int b, int transparent)
{
	...
	/*
	gApplicationLog.printf("[TODO]render_DrawRectangle");
	...
	*/
	...
	for(i = x; i < x + width + 1; i++)
		render_DrawLine(i,y,i,y + height,r,g,b,transparent);
}
...
// * Reset animation
void CAnimationModifier_AlphaFadeIn::Reset(int pParam1, int pParam2, int pParam3)
{
	int i;
	for(i = 0; i < 4; i++)
	{
...
// * Reset animation
void CAnimationModifier_VScan::Reset(int pParam1, int pParam2, int pParam3)
{
	int i;
	for(i = 0; i < 4; i++)
	{
...
void CAnimationModifier_VScan::UpdateFrame()
{
	...
	if(mStarted == 1)
	{
		...
		if(mVertexList[2].dy >= g_screen_height) {Revert();}
		if(mVertexList[2].dy <= 0.0f) {Revert(); }
	}
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// * Animation : Vertical scan
...
// * Reset animation
void CAnimationModifier_VScan2::Reset(int pParam1, int pParam2, int pParam3)
{
	int i;
	for(i = 0; i < 4; i++)
	{
...
// * Reset animation
void CAnimationModifier_HScan::Reset(int pParam1, int pParam2, int pParam3)
{
	int i;
...
// * Attach image vertices and initialize
void CAnimationModifier_HScan2::Attach(TVertex* pVertexList)
{
	mVertexList = pVertexList;
	mSpeed      = 300.0f;
	mStarted    = 0;	
...
// * Reset animation
void CAnimationModifier_HScan2::Reset(int pParam1, int pParam2, int pParam3)
{
	int i;
...
// * Start animation
void CAnimationModifier_HScan2::Start()
{	
	mStarted   = 1;
	mDirection = 1.0f;
}
// * Stop animation
void CAnimationModifier_HScan2::Stop()
{
	mStarted = 0;
}
// * Revert animation
void CAnimationModifier_HScan2::Revert()
{
	mDirection *= -1.0f;
}
// * Update frame
void CAnimationModifier_HScan2::UpdateFrame()
{
	int i;
...
// * Attach image vertices and initialize
void CAnimationModifier_FlipIn::Attach(TVertex* pVertexList)
{
	mVertexList = pVertexList;
	mSpeedX      = 0.0f;
	mSpeedY      = 0.0f;
...
// * Reset animation
void CAnimationModifier_FlipIn::Reset(int pParam1, int pParam2, int pParam3)
{
	int i;
...
// * Start animation
void CAnimationModifier_FlipIn::Start()
{	
	mStarted   = 1;
	mCount     = 0;
}
// * Stop animation
void CAnimationModifier_FlipIn::Stop()
{
	mStarted = 0;
}
// * Revert animation
void CAnimationModifier_FlipIn::Revert()
{
	mDirection *= -1.0f;
}
// * Update frame
void CAnimationModifier_FlipIn::UpdateFrame()
{
	int    i;
	double temp;
...
// * Reset animation
void CAnimationModifier_ScaleIn::Reset(int pParam1, int pParam2, int pParam3)
{
	int i;
...
// * Reset animation
void CAnimationModifier_ScaleInH::Reset(int pParam1, int pParam2, int pParam3)
{
	int i;
...
// * Reset animation
void CAnimationModifier_ScaleInHCenter::Reset(int pParam1, int pParam2, int pParam3)
{
	int i;
...
{							
	...
	ypos    = y - (16 - m_FontSize) / 2;
	ypos = y;

	// 2014 update
	if(fontSize == 10) ypos = y - 7;
	if(fontSize == 12) ypos = y - 2;
	...
	{
		// Text is displayed in the console highlight color
		if(*fmt == '/')
		{
			index += 1;
			SDL_SetTextureColorMod(m_FontTexture,gSkinColorConsoleHighlight.r,gSkinColorConsoleHighlight.g,gSkinColorConsoleHighlight.b);
		}
		else
