...
int main( int argc, char* args[])
{
	...
	else
	{
		...
		// Fallback - Sometimes SDL suggest a resolution higher than the current one.
		if(closest.h > current.h)
		{
			closest.h = current.h;
			closest.w = current.w;
		}
	...
	if(gFullScreen == 1)
	{
		sdlWindow   = SDL_CreateWindow("Hacker Evolution", (current.w - 1280) / 2, (current.h - 800) / 2, 1280, 800, SDL_WINDOW_SHOWN);
		g_screen_width  = 1280;
		g_screen_height = 800;
	}
	else
	...
	do
	{
		...
		while (SDL_PollEvent(&sdlEvent))
		{
			...
			{
				if(sdlEvent.button.button == SDL_BUTTON_LEFT)  _mouseLBDown = 0;
