...
void heGameWindow::Create(int x, int y, int width, int height, char* imagePath, int pCaption)
{
    ...
    //m_textWidth  = (width  / 6);
	m_textWidth  = 190;
    m_textHeight = (height / FONT_SIZE) - 2;
...
void heGameWindow::renderTextContent()
{
	// render text content 
    int i,j;
	char line[1000];
    ...
    for(i = starty; i <= endy; i++)
	{
        ...
        if(line[0] != 0)
		    render_DrawText(x_ofs ,y_ofs + (i - starty) * FONT_SIZE,line,SKINCOLOR(gSkinColorWindowText));
	}
}
// ============================================================================================================================
...
void heGameWindow::printText(int x, int y, int r, int g, int b, char* fmt, ...)
{
	...
	render_DrawText(this->m_x + x, this->m_y + y,buffer,r,g,b,0);
}
